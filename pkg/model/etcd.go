...
func NewEtcdStore(etcdAddrs []string, prefix string) (Store, error) {
	...
	store.closed = false
	store.cli = etcd.NewClient(etcdAddrs)
	store.watchMethodMapping = make(map[EvtSrc]func(EvtType, *etcd.Response) *Evt)
	store.init()
	return store, nil
}
...
func (e *EtcdStore) SaveAPI(api *API) error {
	key := fmt.Sprintf("%s/%s", e.apisDir, getAPIKey(api.URL))
...
func (e *EtcdStore) DeleteAPI(apiURL string) error {
	return e.deleteKey(getAPIKey(apiURL), e.apisDir, e.deleteAPIsDir)
}
...
func (e *EtcdStore) GetAPIs() ([]*API, error) {
	...
	for i := 0; i < l; i++ {
		v, marshalerr := UnMarshalAPI([]byte(rsp.Node.Nodes[i].Value))
		if marshalerr != nil {
			log.Errorf("marshal api value is error %s", rsp.Node.Nodes[i].Value)
			continue
		}
		v.Key = rsp.Node.Nodes[i].Key
		apis[i] = v
	}
...
func (e *EtcdStore) GetAPI(apiURL string) (*API, error) {
	key := fmt.Sprintf("%s/%s", e.apisDir, getAPIKey(apiURL))
	rsp, err := e.cli.Get(key, false, false)

	if nil != err {
		return nil, err
	}
	v, marshalerr := UnMarshalAPI([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("marshal api value is error %s", rsp.Node.Value)
...
func (e *EtcdStore) DownServer(addr string) error {
	old, err := e.GetServer(addr)
...
func (e *EtcdStore) GetServers() ([]*Server, error) {
	...
	for i := 0; i < l; i++ {
		v, marshalerr := UnMarshalServer([]byte(rsp.Node.Nodes[i].Value))
		if marshalerr != nil {
			log.Errorf("marshal server value is error %s", rsp.Node.Nodes[i].Value)
			continue
		}
		v.Key = rsp.Node.Nodes[i].Key
		servers[i] = v
...
func (e *EtcdStore) GetServer(serverAddr string) (*Server, error) {
	...
	v, marshalerr := UnMarshalServer([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("marshal server value is error %s", rsp.Node.Value)
		return nil, marshalerr
	}
	v.Key = key
	return v, nil
...
func (e *EtcdStore) GetClusters() ([]*Cluster, error) {
	...
	for i := 0; i < l; i++ {
		v, marshalerr := UnMarshalCluster([]byte(rsp.Node.Nodes[i].Value))
		if marshalerr != nil {
			log.Errorf("marshal cluster value is error %s", rsp.Node.Nodes[i].Value)
			continue
		}
		clusters[i] = v
		v.Key = rsp.Node.Nodes[i].Key
...
func (e *EtcdStore) GetCluster(clusterName string) (*Cluster, error) {
	...
	v, marshalerr := UnMarshalCluster([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("marshal cluster value is error %s", rsp.Node.Value)
		return nil, marshalerr
	}
	v.Key = key
	return v, nil
...
func (e *EtcdStore) GetRoutings() ([]*Routing, error) {
	...
	for i := 0; i < l; i++ {
		v, marshalerr := UnMarshalRouting([]byte(rsp.Node.Nodes[i].Value))
		if marshalerr != nil {
			log.Errorf("marshal cluster value is error %s", rsp.Node.Nodes[i].Value)
			continue
		}
...
func (e *EtcdStore) doWatchWithCluster(evtType EvtType, rsp *etcd.Response) *Evt {
	cluster, marshalerr := UnMarshalCluster([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("doWatchWithCluster marshal is error: %s", rsp.Node.Value)
	}
...
func (e *EtcdStore) doWatchWithServer(evtType EvtType, rsp *etcd.Response) *Evt {
	server, marshalerr := UnMarshalServer([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("doWatchWithServer marshal is error: %s", rsp.Node.Value)
	}
	return &Evt{
		Src:   EventSrcServer,
		Type:  evtType,
...
func (e *EtcdStore) doWatchWithAPI(evtType EvtType, rsp *etcd.Response) *Evt {
	api, marshalerr := UnMarshalAPI([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("doWatchWithAPI marshal is error: %s", rsp.Node.Value)
	}
...
func (e *EtcdStore) doWatchWithRouting(evtType EvtType, rsp *etcd.Response) *Evt {
	routing, marshalerr := UnMarshalRouting([]byte(rsp.Node.Value))
	if marshalerr != nil {
		log.Errorf("doWatchWithRouting marshal is error: %s", rsp.Node.Value)
	}
	return &Evt{
		Src:   EventSrcRouting,
		Type:  evtType,
