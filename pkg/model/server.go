...
type Server struct {
	...
	CheckPath string `json:"checkPath,'system'"`
...
func UnMarshalDelServer(path, addr string) *Server {
	v := &Server{Key: path, Addr: addr}
	return v
}
...
func (s *Server) updateFrom(svr *Server) {
	...
	s.CheckDuration = svr.CheckDuration
	s.CheckPath = svr.CheckPath
