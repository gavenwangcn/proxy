...
func (r *RouteTable) ChangeProxyCheck(proxyCheck bool) {
	r.proxyCheck = proxyCheck
}
...
func (r *RouteTable) UpdateAPI(api *API) error {
	...
	if _, ok := r.apis[getAPIKey(api.URL)]; !ok {
...
func (r *RouteTable) DeleteAPI(api *API) error {
	r.rwLock.Lock()
	defer r.rwLock.Unlock()
	...
	delete(r.apis, getAPIKey(api.URL))

	log.Infof("API <%s> deleted", api.URL)
...
func (r *RouteTable) UpdateServer(svr *Server) error {
	...
	// 根据zk的status 判断server状态 进行bind 或unbind
	status := svr.Status
	r.checkStatusByZk(status, old)
...
func (r *RouteTable) Bind(svrAddr string, clusterName string) error {
	...
	if !ok {
		log.Debugf("Bind <%s,%s> fail: %s", svrAddr, clusterName, ErrServerNotFound.Error())
	...
	cluster, ok := r.clusters[clusterName]
	if !ok {
		log.Debugf("Bind <%s,%s> fail: %s", svrAddr, clusterName, ErrClusterNotFound.Error())
	...
	if ok && bindCluster.Name == clusterName {
		log.Debugf("Bind <%s,%s> fail: %s", svrAddr, clusterName, ErrBindExists.Error())
...
func (r *RouteTable) Select(req *fasthttp.Request) []*RouteResult {
	...
	//添加快速匹配 不进行正则 通过添加路径+/*方式 快速查找api 而不进行正则匹配
	fastResults := r.FastSelect(req)
	if fastResults != nil {
		log.Debugf("Go to FastSelect url %s", req.URI().RequestURI())
		r.rwLock.RUnlock()
		return fastResults
	}
...
// FastSelect return route result
//
func (r *RouteTable) FastSelect(req *fasthttp.Request) []*RouteResult {
	uri := req.URI().RequestURI()
	api := r.findApiByRoot(uri, r.apis)
	if api != nil && api.isMethodMatches(req) {
		var results []*RouteResult
		results = make([]*RouteResult, len(api.Nodes))
		for index, node := range api.Nodes {
			results[index] = &RouteResult{
...
func (r *RouteTable) findApiByRoot(rootApi []byte, apis map[string]*API) *API {
	uri := string(rootApi)
	var api *API
	keysField := strings.SplitAfter(uri, "/")
	rootField := ""
	if len(keysField) > 0 {
		//对最后一次Field"/"不进行取出
		for i := 0; i < len(keysField)-1; i++ {
			rootField = rootField + keysField[i]
			getApi := apis[getAPIKey(rootField+"*")]
			if getApi != nil && getApi.isUp() {
				//取最近路径api 只是增加了循环次数，但是可以区分子节点配置
				api = getApi
			}
		}
	}
...
func (r *RouteTable) doReceiveRouting(evt *Evt) {
	...
	if evt.Type == EventTypeNew {
		...
		// TODO: impl
	} else if evt.Type == EventTypeReload {
		r.reLoadRoutings()
	}
}
...
func (r *RouteTable) loadClusters() {
	...
	for _, cluster := range clusters {
		if cluster != nil {
			...
			if nil != err {
				log.WarnErrorf(err, "Server <%s> add fail.", cluster.Name)
			}
		}
	}
}
...
func (r *RouteTable) loadServers() {
	...
	if nil != err {
		log.WarnErrorf(err, "Load servers fail.")
		return
	}
	...
	for _, server := range servers {
		if server != nil {
			...
			if nil != err {
				log.WarnErrorf(err, "Server <%s> add fail.", server.Addr)
			}
		}
	}
}
...
func (r *RouteTable) loadRoutings() {
	...
	if nil != err {
		log.WarnErrorf(err, "Load routings fail.")
		return
	}
	...
	for _, route := range routings {
		if route != nil {
			...
			if nil != err {
				log.WarnErrorf(err, "Routing <%s> add fail.", route.Cfg)
			}
		}
	}
}
...
func (r *RouteTable) loadBinds() {
	...
	if nil != err {
		log.WarnErrorf(err, "Load binds fail.")
		return
	}
...
func (r *RouteTable) loadAPIs() {
	...
	if nil != err {
		log.WarnErrorf(err, "Load apis fail.")
		return
	}
	...
	for _, api := range apis {
		if api != nil {
			err := r.AddNewAPI(api)
			if nil != err {
				log.WarnErrorf(err, "API <%s> add fail.", api.URL)
			}
		}
	}
}

func (r *RouteTable) reLoadClusters() {
	clusters, err := r.store.GetClusters()
	if nil != err {
		log.WarnErrorf(err, "reLoad clusters fail.")
		return
	}
	...
	for _, cluster := range clusters {
		if cluster != nil {
			err := r.AddNewCluster(cluster)
			if nil != err {
				log.Debugf("cluster <%s> have", cluster.Key)
			} else {
				zkStore, _ := r.store.(*ZkStore)
				zkStore.cli.WatchData(cluster.Key)
			}
		}
	}
}

func (r *RouteTable) reLoadServers() {
	servers, err := r.store.GetServers()
	if nil != err {
		log.WarnErrorf(err, "Load servers fail.")
		return
	...
	for _, server := range servers {
		if server != nil {
			err := r.AddNewServer(server)
			if nil != err {
				log.Debugf("server <%s> have", server.Key)
			} else {
				zkStore, _ := r.store.(*ZkStore)
				zkStore.cli.WatchData(server.Key)
			}
		}
	}
}

func (r *RouteTable) reLoadRoutings() {
	routings, err := r.store.GetRoutings()
	if nil != err {
		log.WarnErrorf(err, "Load routings fail.")
		return
	...
	for _, route := range routings {
		if route != nil {
			err := r.AddNewRouting(route)
			if nil != err {
				log.Debugf("route <%s> have", route.Key)
			} else {
				zkStore, _ := r.store.(*ZkStore)
				zkStore.cli.WatchData(route.Key)
			}
		}
	}
}

func (r *RouteTable) reLoadBinds() {
	binds, err := r.store.GetBinds()
	if nil != err {
		log.WarnErrorf(err, "Load binds fail.")
		return
	...
	for _, b := range binds {
		if b != nil {
			err := r.Bind(b.ServerAddr, b.ClusterName)
			if nil != err {
				log.Debugf("bind <%s> have", b.Key)
			} else {
				zkStore, _ := r.store.(*ZkStore)
				zkStore.cli.WatchData(b.Key)
			}
		}
	}
}

func (r *RouteTable) reLoadAPIs() {
	apis, err := r.store.GetAPIs()
	if nil != err {
		log.WarnErrorf(err, "Load apis fail.")
		return
	...
	for _, api := range apis {
		if api != nil {
			err := r.AddNewAPI(api)
			if nil != err {
				log.Debugf("api <%s> have", api.Key)
			} else {
				zkStore, _ := r.store.(*ZkStore)
				zkStore.cli.WatchData(api.Key)
			}
		}
	}
}
...
func (r *RouteTable) checkStatusByZk(status Status, svr *Server) {
	if svr.Status != status {
		svr.changeTo(status)
		r.evtChan <- svr
	}
...
func (r *RouteTable) check(addr string) {
	...
	if svr.check(r.addToCheck) {
		...
		if svr.statusChanged() {
			log.Infof("Server <%s, %s> DOWN.", svr.Addr, svr.CheckPath)
		}
	}
...
func (r *RouteTable) DownServer(svr *Server) {
	zkStore, _ := r.store.(*ZkStore)
	svr.changeTo(Down)
	zkStore.DownServer(svr.Addr)
	r.evtChan <- svr
}

func (r *RouteTable) FlushSvrsChecks(flushDuration int) {
	const DefaultFlushDuration = 600
	if flushDuration == 0 {
		flushDuration = DefaultFlushDuration
	}
	for {
		for _, s := range r.svrs {
			s.CheckFailCount = 0
		}
		time.Sleep(time.Duration(flushDuration) * time.Second)
	}
